...
typedef struct puntata{// struct contenente la coppia valore/tipo di puntata
    char tipo;
    int valore;
	struct puntata *succ;
} punt; /* L: Una typedef è migliore se fatta così, più leggibile e puoi
			  vedere immediatamente la sua struct*/

void n_puntata(punt *l);
int  main() {
  punt *np; /*L: corretta la definizione di np, cosa sgnifica?*/
  char sc;
  int dsp=10000;
  char gc;
  ...
  int riga3[]={3,6,9,12,15,18,21,24,27,30,33,36}; L: a mio avviso non servono, si possono tutti calcolare facilmente*/
